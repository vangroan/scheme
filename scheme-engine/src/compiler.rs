...
pub fn compile(env: Env, expr: &Expr) -> Result<Handle<Closure>> {
    ...
    let closure = Closure::new(Handle::new(proc));
...
impl Compiler {
    fn take_procedure(self) -> Result<(Env, Proc)> {
    ...
    fn compile_call(&mut self, list: &[Expr]) -> Result<()> {
        ...
        match &list[0] {
            Expr::Ident(ident) => {
                ...
                let value = self.env.get_var(symbol).cloned().unwrap();
                self.proc.emit_op(Op::LoadEnvVar(symbol));
